...
_.isPancard = function (pancard) {
    const regeX = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
    return !regeX.test(pancard);
};
...
_.randomizeAlphaNumericString = function (length, size) {
    let result = '';
...
_.getJoker = function (array) {
    let randomIndex = Math.floor(Math.random() * array.length);
    let oWildJoker = array[randomIndex];
    if (oWildJoker.eSuit === 'j') randomIndex = array.findIndex(c => c.nLabel === 1 && c.eSuit === 's');
    oWildJoker = array[randomIndex];
...
_.getRandomNumberKey = () => 'private-codes';

_.getTournamentTableKey = iTableId => `${iTableId}:tbl`;

_.getTournamentTableCounterKey = id => `${id}:counter`;

_.getFinancialYear = () => {
    const today = new Date();
    const fiscalyear = today.getMonth() + 1 <= 3 ? `${today.getFullYear() - 1}-${today.getFullYear()}` : `${today.getFullYear()}-${today.getFullYear() + 1}`;
    const dFrom = new Date(`4-1-${fiscalyear.split('-')[0]}`).toLocaleString();
    const dTo = new Date(`3-31-${fiscalyear.split('-')[1]}`).toLocaleString();
    return { dFrom, dTo, fiscalyear };
};
